# Documentação de Arquitetura e Modelagem: Módulo de Persistência Local (Armazenamento Interno) 
...
O `sqflite` se comunica nativamente com dados estruturados na forma de Linha/Coluna (`Map<String, dynamic>`). Abaixo, o diagrama ilustra o ciclo de vida e a transformação sofrida pelo dado desde a memória da aplicação (Objeto) até o disco de armazenamento (Tabela SQLite).
...
O Banco de Dados SQLite armazena a estrutura da tabelsa utilizando restrições (constraints) e tipos primitivos de dados relacionais
